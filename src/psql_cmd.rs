...
";
    ...
    fn test_table_token_savings() {
        // Uses real-format fixture (wide padded table).
        // Table-to-tab conversion saves ~40% (removes `|` delimiters, separator lines, footer).
        // 60%+ is not achievable by tab conversion alone: `|` tokens represent at most ~47% of
        // input tokens regardless of column count. Expanded format achieves 60%+ by compressing
        // verbose `-[ RECORD N ]` headers + field label padding.
        let input = include_str!("../tests/fixtures/psql_table.txt");
        let result = filter_table(input);
    ...
    fn test_expanded_token_savings() {
        // Uses real-format fixture (expanded display with many fields) to verify ≥60% savings
        let input = include_str!("../tests/fixtures/psql_expanded.txt");
        let result = filter_expanded(input);
        let input_tokens = count_tokens(input);
        let output_tokens = count_tokens(&result);
